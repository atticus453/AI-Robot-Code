...
public:
    ...
    float get_joint(float *current_pos,float*current_torque,OD_Motor_Msg * motor_msg,bool &current_normal);
    bool temp_condition=true;
    int temp_current_normal=0;
    float torque_temp[10]={};

    float ramp(float goal, float current, float ramp_k);
    float target_pos_t[7] = {0.0f};
    float control(float*target_pos_r,float*get_pos,bool& motor_control_cmd,bool& is_teach_mode,bool& current_normal);
    float control2(float*target_pos,bool& motor_control_cmd,bool& is_teach_mode,bool& current_normal);
    can CAN_Handlej;
    float arx_1();
    float arx_2();
